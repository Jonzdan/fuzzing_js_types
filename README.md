...
5. Run `npx tsx fuzzer/process-dir.ts` to take fuzzed inputs in `/corpus` and write tests to `/tests`
6. Run `node scripts/run-tests.js` to run type inference on the corpus and generate `.d.ts` declaration files, output to `/seeds/`